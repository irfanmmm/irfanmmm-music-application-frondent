...
const MusicPlayer = ({navigation}) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      if (queee.length > 0) {
        console.log(
          selectedIndex && selectedIndex?.toString()?.length > 0,
          'countition satisfied====',
        );
  ...
  const minHeight = 0; // Starting height (collapsed state).
  const maxHeight = hp(-97);
  const gesture = Gesture.Pan()
    .onStart(() => {})
    .onUpdate(event => {
      const newHeight = minHeight - event.translationY;

      if (Math.round(newHeight) > 10) {
